  ...
  return (
    <div className="overflow-x-auto rounded-xl shadow bg-white">
      <table className="w-full text-sm text-left">
        ...
        <tbody>
          {leaves.map((leave, i) => (
            <tr key={leave._id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
              <td className="px-4 py-3 font-medium text-slate-800">{leave.userId?.fullName ?? '—'}</td>
              ...
              <td className="px-4 py-3 text-slate-500 text-xs max-w-[200px] truncate">{leave.reason || '—'}</td>
